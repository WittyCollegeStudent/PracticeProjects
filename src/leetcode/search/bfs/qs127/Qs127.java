package leetcode.search.bfs.qs127;
...
class Solution {
    ...
    public int ladderLength(String beginWord, String endWord, List<String> wordList) {
        if (beginWord == null || beginWord.length() == 0 || wordList == null || wordList.size() == 0
                || Objects.equals(beginWord, endWord)) {
            return 0;
        }
        ...
        int startIndex;
        if ((startIndex = wordList.indexOf(beginWord)) < 0) {
            wordList.add(beginWord);
            startIndex = wordList.size() - 1;
        }
        int lenth = 1;
        boolean[] hasVisited = new boolean[wordList.size()+1];
        // 生成diffs
        List<List<Integer>> tryLists = generateNextIndexes(wordList);
        Queue<Integer> nextWordIndexList = new LinkedList<>();
        nextWordIndexList.add(startIndex);
        hasVisited[startIndex] = true;
        while (!nextWordIndexList.isEmpty()) {
            int size = nextWordIndexList.size();
            ++ lenth;
            while (size-- > 0) {
                int currWordIndex = nextWordIndexList.poll();
                List<Integer> tryList = tryLists.get(currWordIndex);
                for (int nextIndex : tryList) {
                    ...
                    nextWordIndexList.add(nextIndex);
                    hasVisited[nextIndex] = true;
                }
//                hasVisited[currWordIndex] = true;
            }
        }
        return 0;
    }
    private List<List<Integer>> generateNextIndexes(List<String> list) {
        int size = list.size();
        ...
        for (int i = 0 ; i < size ; i ++) {
            ...
            for (int j = 0 ; j < size ; j ++) {
                ...
                // 差异数量
                int cnt = 0;
                for (int m = 0 ; m < item1.length() && cnt < 2 ; m ++) {
                    if (item1.charAt(m) != item2.charAt(m)) {
                        cnt += 1;
                    }
                }
                if (cnt == 1) {
